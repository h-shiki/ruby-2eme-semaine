...
#puts liste.grep(/\_/).count
#array.count { |x| x[/_/]}

# 1 prendre le nombre de journaliste : 
twitter.length 
...
#Combien contiennent au moins une majuscule ?
twitter.count { |x| x[/[A-Z]/]}

#Combien y a-t-il de underscore _ dans tous les pseudos confondus ?
twitter.count { |x| x[/_/]}

#Trie la liste de handle par ordre alphabétique. .sort classera tout seul par ordre alphabétique 
#puts twitter.sort_by(&:downcase)

#Quels sont les 50 handles les plus courts de ce array ?
table_sorted = twitter.sort_by!{|x| x.length}
